...
# not allowed to use itertools >:( , so let's make one!
# algorithm taken from python docs with very minor modifications
...
#             it somehow performed much worse than this so yeah ¯\_(ツ)_/¯
...
# solver
# note: can only search for one solution due to formatting and time constraints
def solve(op, res, char, first, list_num):
    ...
    for perm in permutations(list_num, len(char)):
        ...
        count += 1
        for num in first:
            if num != 0:    # first digit of operands can't be 0
                total_op = 0
                total_res = 0
                for string in op:
                ...
                if total_op == total_res and len(str(total_res)) == len(res[0]):
                    ...
                    for i in range(len(op)):
                        if i == len(op) - 1:
                            print(to_integer(op[i], dict_sol), "+", sep='')
                        else:
                            print(to_integer(op[i], dict_sol))
                    print("------")
                    print(total_res)
                    return count
...
if __name__ == "__main__":
    ...
    # print performance
    print("\nWaktu Eksekusi Program:", time.time() - start, "detik")
